...
public class CustomDialog {
    ...
    ImageView iv_icon;
    String positiveFullBtnTxt, negativeFullBtnTxt;
    ...
    public void setFullButton(String positiveFullBtnTxt, @Nullable final Closure posSelectedNo, String negativeFullBtnTxt, @Nullable final Closure negSelectedNo) {
        this.positiveFullBtnTxt = positiveFullBtnTxt;
        this.negativeFullBtnTxt = negativeFullBtnTxt;
        ...
        if (Utils.checkText(positiveFullBtnTxt)) {
            verticalButtonArea.setVisibility(View.VISIBLE);
            tv_Positive_Area.setVisibility(View.VISIBLE);
            tv_Positive_Button.setText(positiveFullBtnTxt);
        } else {
            tv_Positive_Area.setVisibility(View.GONE);
        }
        if (Utils.checkText(negativeFullBtnTxt)) {
            verticalButtonArea.setVisibility(View.VISIBLE);
            tv_Negative_Area.setVisibility(View.VISIBLE);
            tv_Negative_Button.setText(negativeFullBtnTxt);
        } else {
            tv_Negative_Area.setVisibility(View.GONE);
        }

        if (Utils.checkText(positiveFullBtnTxt)) {
            tv_Positive_Button.setOnClickListener(view -> {
                if (posSelectedNo != null) {
                    posSelectedNo.exec();
                }
                closeDialog();
            });
    ...
    protected void createCustomDialog() {
        ...
        final View imgBgArea = customDialog.findViewById(R.id.imgBgArea);
...
//        cv_detailArea.setBackgroundColor(mContext.getResources().getColor(cardAreaColor));
        if (iconTintColor > 0) {
            iv_icon.setColorFilter(ContextCompat.getColor(mContext, iconTintColor));
        }
        if (isAlpha) {
            ...
            LinearLayout.LayoutParams mParams = (LinearLayout.LayoutParams) iv_icon.getLayoutParams();
            mParams.width = mContext.getResources().getDimensionPixelSize(R.dimen._50sdp);
            mParams.height = mContext.getResources().getDimensionPixelSize(R.dimen._50sdp);
            iv_icon.setLayoutParams(mParams);
        }
