...
public class PackageAdapter extends RecyclerView.Adapter<PackageAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(PackageAdapter.ViewHolder holder, final int position) {
        ...
        if (!isPositionChanged && Utils.checkText(map.get("isInterCity")) && map.get("isInterCity").equalsIgnoreCase("true")) {
            pos = -1;
            if (Utils.checkText(map.get("isSelected")) && map.get("isSelected").equalsIgnoreCase("yes")) {
                pos = position;
            }
        }
