...
import com.view.bottombar.ext.d2p
...
class SmoothBottomBar @JvmOverloads constructor(
    ...
    var itemMenuRes: Int
        ...
        set(value) {/*if (value != INVALID_RES && (items == null || items.isEmpty())) {
                _itemMenuRes = value
    ...
    fun initBottomBar(bottomBarItems: ArrayList<BottomBarItem>) {
        if (items != null && items.isNotEmpty()) {
            items.clear()
        }
        items = BottomBarParser(context, bottomBarItems).parse()

        val popupMenu = PopupMenu(context, null)

        for (i in items.indices) {
            val bootom_bar_item = items[i];
            popupMenu.menu.add(1, Utils.generateViewId(), i, bootom_bar_item.title)
        }
        ...
        initializeTouchHelper(attrs, this.defStyleAttr!!)
    ...
    private var exploreByTouchHelper: AccessibleExploreByTouchHelper? = null
    ...
    init {
        ...
        initializeTouchHelper(attrs, defStyleAttr)
    }

    private fun initializeTouchHelper(attrs: AttributeSet?, defStyleAttr: Int) {
...
//            )
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        val itemsToLayout =
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1 && layoutDirection == LAYOUT_DIRECTION_RTL) items.reversed() else items
