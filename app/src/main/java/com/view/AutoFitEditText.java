...
public class AutoFitEditText extends AppCompatEditText {
    ...
    //
    public static String convertCommaToDecimal(@NonNull String strVal, boolean isGet) {
        if (MyApp.getInstance().getAppLevelGeneralFunc().retrieveValue("eReverseformattingEnable").equalsIgnoreCase("Yes")) {
            if (isGet) {
    ...
    public void setHint(String hint) {
        super.setHint(convertCommaToDecimal(hint, false));
    }

    public void setText(String text) {
        super.setText(convertCommaToDecimal(text, false));
    }

    @NonNull
    public String getTxt() {
        return convertCommaToDecimal(Objects.requireNonNull(getText()).toString(), true);
    }

    public void addTextChangedListener(@Nullable MButton btn, boolean isEquals) {
        super.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (btn != null) {
                    if (Utils.checkText(Objects.requireNonNull(getText()).toString())) {
                        if (isEquals) {
                            btn.setEnabled(GeneralFunctions.parseDoubleValue(0, getText().toString()) >= 0);
                        } else {
                            btn.setEnabled(GeneralFunctions.parseDoubleValue(0, getText().toString()) > 0);
                        }
                    } else {
