...
public class RecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements KmRecyclerView.KmStickyListener {
    ...
    public int getItemViewType(int position) {
        if (mCountryList != null && mCountryList.size() > 0 && mCountryList.get(position) != null && Utils.checkText(mCountryList.get(position).getmViewType()) && mCountryList.get(position).getmViewType().equalsIgnoreCase("Header")) {
            return TYPE_HEADER;
        }
        return TYPE_ITEM;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof CountryHolder) {
            ...
            new LoadImageGlide.builder(holder.itemView.getContext(), LoadImageGlide.bind(mCountryList.get(position).getFlagName()), countryholder.mImageViewFlag).setErrorImagePath(R.color.imageBg).setPlaceholderImagePath(R.color.imageBg).build();
    ...
    static class CountryHolder extends RecyclerView.ViewHolder {
        ...
        private SelectableRoundedImageView mImageViewFlag;
